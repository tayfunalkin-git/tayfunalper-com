...
public partial class ivfyonetici_talper : System.Web.UI.Page
{
    ...
    private void listeYukle()
    {
        ...
        string sql = "Select H.uye_id as HID,cast(sum(M.faturasiKesilen) as UNSIGNED) as toplam,Concat(H.uye_adi,' ',H.uye_soyadi) as adi,'muhasebe' as tur from muhasebe M left join uye_hasta H on M.hastaID=H.uye_id " + kaynakEkTablo + " where " + kaynakEkWhere + "";
   
        if (ay.SelectedValue == "")
        {
            sql += " YEAR(str_to_date(M.tarih ,'%d.%m.%Y')) = " + yil.SelectedValue + " and M.kontrol='+' and M.faturasiKesilen <> 0 and";
        }
        else
        {
            sql += " YEAR(str_to_date(M.tarih ,'%d.%m.%Y')) = " + yil.SelectedValue + " and MONTH(str_to_date(M.tarih ,'%d.%m.%Y'))=" + ay.SelectedValue + " and M.kontrol='+' and M.faturasiKesilen <> 0 and";
        }

        if (doktor.SelectedIndex != 0)
        {
            sql += " M.doktorID = '" + doktor.SelectedItem.Text + "' and";
        }


        if (kayitTuru.SelectedIndex != 0)
        {
            sql += " M.tur = '" + kayitTuru.SelectedValue + "' and";
        }
        else
        {
            sql += " M.tur <> '4' and";
        ...
        da2.SelectCommand.CommandTimeout = 1200;
        ...
        da3.SelectCommand.CommandTimeout = 1200;
        ...
        da4.SelectCommand.CommandTimeout = 1200;
        ...
        da5.SelectCommand.CommandTimeout = 1200;
        ...
        da6.SelectCommand.CommandTimeout = 1200;
        ...
        foreach (GridViewRow satir in liste2.Rows)
        {
            ...
            lblAdiSoyadi.OnClientClick = "createWindow('http://ivf2.ebebaba.com/ivfyonetici/hasta_modulu/th4820ft.aspx?ID=" + sDt.Rows[m][0].ToString() + "','" + sDt.Rows[m][1].ToString() + "','90%','530','');return false;";
