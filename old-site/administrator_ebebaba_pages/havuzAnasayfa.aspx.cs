...
public partial class mainAdminPagesHdn_havuz : System.Web.UI.Page
{
    ...
    private void resimYukle()
    {
        ...
        MySqlDataAdapter da6 = new MySqlDataAdapter("Select * from resimhavuzuanasayfa where onay='+' order by tarih desc", connect_word);
        ...
        foreach (DataListItem oge in resimList.Items)
        {
            ...
            resim.Text = "<div class='hovergallery'><img onclick='ekle(this.title,this.alt);' width='225px' alt='"+Request.QueryString["gonderen"].ToString()+"' title='" + dt6.Rows[a]["adi"].ToString() + "' src='../uploadImages/" + dt6.Rows[a]["adi"].ToString() + "_450.jpg'/></div>";
    ...
    protected void sil_Command(object sender, CommandEventArgs e)
    {
        ...
        MySqlConnection connect_word = noc.connect_ebebaba(0301009184);
        MySqlCommand cmd = new MySqlCommand("delete from  resimhavuzuanasayfa where ID=" + e.CommandArgument.ToString() + "", connect_word);
    ...
    {
        Response.Redirect("resimYukleAnasayfa.aspx?gonderen=" + Request.QueryString["gonderen"].ToString());
    }
}