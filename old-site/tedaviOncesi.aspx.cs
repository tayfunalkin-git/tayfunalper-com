...
public partial class makaleGoruntule : System.Web.UI.Page
{
    ...
    private void siralamaYukle()
    {
        ...
        {
            DataRow[] devamsatiri = dtm.Select("link='tup-bebek-tedavi-oncesi-" + Request.QueryString["ID"].ToString() + ".aspx'");
    ...
    void konuYukle(string ID)
    {
        try
        {
            ...
            if (dt_kitap.Rows.Count != 0)
            {
                ...
                icerik.InnerHtml = "<div style='margin-bottom:18px;'><h3>" + dt_kitap.Rows[0]["baslik"].ToString() + "</h3></div>" + icerikMetni;
